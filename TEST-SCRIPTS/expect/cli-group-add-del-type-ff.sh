...
for {set i 1} {$i <= $range} {incr i 1} {
    send "of-group add switch $dpid group $i type ff\r"
    expect "#"
    send "group-act-vector ff-group 1\r"
    expect "#"
    send "group-act-vector ff-port 2\r"
    expect "#"
    send "action-add set-dmac 00:02:03:04:05:01\r"
    expect "#"
