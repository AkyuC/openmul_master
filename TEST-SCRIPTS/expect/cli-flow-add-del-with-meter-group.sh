...
for {set i 1} {$i <= $range } {incr i 1} {
    send "of-meter add switch $dpid meter-id $i meter-type kbps burst yes stats no\r"
    expect "#"
    send "meter-band dscp-remark rate 4096 burst-size 1024 prec-level 1\r"
    expect "#"
    send "meter-band-next\r"
    expect "#"
    send "meter-band drop rate 2222 burst-size 222\r"
    expect "#"
    send "commit-meter\r"
    expect "#"
#Add groups
...
for {set i 0} {$i <= $range / 255 } {incr i 1} {
    ...
    expect "#"
    set meter "[ expr $i * 255 + $j + 1]"
    send "instruction-meter $meter\r"
    expect "#"
    send "instruction-apply\r"
    expect "#"
    send "action-add group-id $meter\r"
    expect "#"
    send "action-add output 2\r"
    expect "#"
...
#delete meters and groups after deleting flows
    for {set i 1} {$i <= $range} {incr i 1} {
        send "of-meter delete switch $dpid meter-id $i\r"
        expect "#"
