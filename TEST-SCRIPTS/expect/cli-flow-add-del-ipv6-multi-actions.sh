...
for {set i 0} {$i <= $range / 255 } {incr i 1} {
    for {set j 0} {$j < 255 && (($i * 255 + $j) < $range) } {incr j 1} {
        set hex_i [format %04X $i]
        set hex_j [format %04X $j]
    send "of-flow add switch $dpid smac 00:01:02:03:04:05 dmac 00:01:02:03:04:06  eth-type 0x86DD vid * vlan-pcp * mpls-label * mpls-tc * mpls-bos * dip6 2001:0db8:0000:0000:0000:$hex_i:0042:$hex_j/128 sip6 2001:0db8:0000:0000:0000:$hex_j:0042:$hex_i/128 proto * tos * dport * sport * in-port * table 0\r"
    expect "#"
    ...
    expect "#"
    send "action-add nw-saddr6 1001:0db8:0000:0000:0000:$hex_i:0042:$hex_j\r"
    expect "#"
    send "action-add nw-daddr6 1001:0db8:0000:0000:0000:$hex_j:0042:$hex_i\r"
    expect "#"
...
if { $del > 0} {
    for {set i 0} {$i <= $range / 255} {incr i 1} {
        ...
        set hex_i [format %04X $i]
        set hex_j [format %04X $j]
    send "of-flow del switch $dpid smac 00:01:02:03:04:05 dmac 00:01:02:03:04:06  eth-type 0x86DD vid * vlan-pcp * mpls-label * mpls-tc * mpls-bos * dip6 2001:0db8:0000:0000:0000:$hex_i:0042:$hex_j/128 sip6 2001:0db8:0000:0000:0000:$hex_j:0042:$hex_i/128 proto * tos * dport * sport * in-port * table 0\r"
        expect "#"
        }
    }
}
